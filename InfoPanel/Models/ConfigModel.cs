            ...
            if (File.Exists(fileToLoad))
            {
                ...
                try
                {
                    if (xs.Deserialize(rd) is Settings settings)
                    {
                        lock (_settingsLock)
                        {
                            // Force startup resolution to 800x600 as requested by the user,
                            // but make sure it never exceeds the screen size in tiny monitors.
                            var screenWidth = (float)System.Windows.SystemParameters.WorkArea.Width;
                            var screenHeight = (float)System.Windows.SystemParameters.WorkArea.Height;
                            
                            Settings.UiWidth = Math.Min(800, screenWidth);
                            Settings.UiHeight = Math.Min(600, screenHeight);
